...
Lorsqu'on spécifie un Content-Type approprié on indique explicitement au navigateur comment interpréter le contenu de la réponse. Par exemple dans le fichier server.json définit un Content-Type: text/html ce qui réduit le risque d'interprétation inattendue ou d'exploitation par des attaquants.
